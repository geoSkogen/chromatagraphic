...
// this is the DOM/view  concern
  ...
  */
  ...
  function calculateResults() {
   ...
   if (controller.lineCalc == true) {
     ...
     operator = false
     falseOperator = false
     //input.value = trace.backtrace[trace.backtrace.length-1]
     return sum
   }
  }
    ...
    switch (index) {
      ...
      default :
        ...
        if (controller.lineCalc) {
          total = trace.backtrace.length >= 4 ? trace.lineCalcSubTotal() : trace.backtrace[0]
          trace.total = total
  ...
  */
  ...
  var controller = { lineCalc : true, base: 10 }
  /*
  // References
  */
  var parenths = ["(",")"]
  ...
  */
  ...
  var input//the actual form input; it's {display:none;}
  var output//the stump onto which new lines of datra are grafted it's .flexOuterColumn
  var results//the modal displaying the sum; it's {display:none;}
  var textBox//the backdrop for the output column
  var subTotal//the modal displaying the sum
  var controlPanel
  var controlPanelConsole
  var currentLine
